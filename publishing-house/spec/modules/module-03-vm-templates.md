...
**Audience**: OCX administrators responsible for defining and publishing standardized VM configurations for tenant use.
...
- Build a VM template from an available base image within the OCX admin console
- Configure template compute resource parameters (CPU, memory, disk)
- Publish the template to an OCX tenant so it is available for VM provisioning

### Lab Structure

| Section | Title | Duration |
|---------|-------|----------|
| 1 | Review available base images | 5 min |
| 2 | Create and configure a VM template | 10 min |
| 3 | Publish the template to the tenant | 5 min |

### Detailed Steps

1. Navigate to the VM Templates section of the OCX admin console and review the available base images.
